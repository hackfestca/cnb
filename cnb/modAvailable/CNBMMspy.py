...
class CNBMMspy(CNBMatrixModule):
    ...
    name = 'spy'
    usage = 'spy MODE [--src SRC1,SRC2 --dst DST1,DST2]'
    desc = 'Useful module to listen to what\'s being said on the internet...'
    aliases = []
    isAdmin = True
    enProcessCmd = True
    ...
    def _initOptParser(self):
        ...
        self.parser.add_option('-l', '--list', action='store_true', dest='mod_list', default=False,
                      help='[mode] List current spying rules')
        self.parser.add_option('-a', '--add', action='store_true', dest='mod_add', default=False,
                      help='[mode] Add a spying rule')
        self.parser.add_option('-d', '--delete', action='store_true', dest='mod_del', default=False,
                      help='[mode] Add a spying rule')
        self.parser.add_option('--src', action='store', type='string', dest='src', default='',
                      help='Specify a list of source')
        self.parser.add_option('--dst', action='store', type='string', dest='dst', default='',
